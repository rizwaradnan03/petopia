...
void SIGNATURE_movement::Execute(SIGNATURE_mesh *mesh) {
  ...
  if (IsKeyDown(KEY_D)) {
    ...
    if (this->get_available_direction()[1] == true) {
      x -= mov;
    }
  }
  ...
  if (this->get_available_direction()[2] == true) {
    if (this->get_elapse_jump() != nullptr) { // move up
      ...
      std::chrono::duration<float> differ =
          currentTime - *this->get_elapse_jump();
...
uint8_t SIGNATURE_movement::get_jump_stock() { return this->jump_stock; }
...
SIGNATURE_movement::get_elapse_jump() {
  return this->elapse_jump;
}

void SIGNATURE_movement::set_elapse_jump(
