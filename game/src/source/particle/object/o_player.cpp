...
OBJECT_player::OBJECT_player(DtoRawMesh rawMesh, DtoCollider *collider)
    : BODY_dynamic(rawMesh, collider) {
  ...
  DtoRawMesh invenCart;
  invenCart.cartesian.x = this->get_mesh()->get_value().x;
  invenCart.cartesian.y = this->get_mesh()->get_value().y;
  invenCart.cartesian.w = 270;
  invenCart.cartesian.h = 120;
  invenCart.rawTextures = {
  ...
  GUI_container *inventory = new GUI_inventory(invenCart, mPoleSet);
  this->set_push_gui_containers(inventory);
}

std::string &OBJECT_player::get_id() { return this->id; }

void OBJECT_player::set_id(std::string value) { this->id = value; }

SIGNATURE_camera *OBJECT_player::get_camera() { return this->camera; }

void OBJECT_player::set_camera(SIGNATURE_camera *value) {
  this->camera = value;
}

SIGNATURE_mesh *OBJECT_player::get_mesh() { return this->mesh; }

void OBJECT_player::set_mesh(SIGNATURE_mesh *value) { this->mesh = value; }

SIGNATURE_movement *OBJECT_player::get_movement() { return this->movement; }

void OBJECT_player::set_movement(SIGNATURE_movement *value) {
  this->movement = value;
}

DtoCollider &OBJECT_player::get_collider() { return *this->collider; }

void OBJECT_player::set_collider(DtoCollider *value) { this->collider = value; }
...
UNI_item *OBJECT_player::get_hold_item() { return this->hold_item; }

void OBJECT_player::set_hold_item(UNI_item *value) { this->hold_item = value; }
